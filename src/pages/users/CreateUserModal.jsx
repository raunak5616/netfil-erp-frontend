...
const CreateUserModal = ({ isOpen, onClose, onSuccess, existingUsers = [] }) => {
  ...
  // Set of employee IDs that already have a User account
  const existingUserEmployeeIds = new Set(
    existingUsers.map((u) => u.employee?._id || u.employee).filter(Boolean)
  );
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === 'username' ? value.toLowerCase().trim() : type === 'checkbox' ? e.target.checked : value,
    }));
  };
  ...
  const validate = () => {
    ...
    if (existingUserEmployeeIds.has(formData.employeeId)) {
      return 'This employee already has an active ERP user account.';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        setSuccessMessage('ERP User account created successfully!');
        setTimeout(() => {
          onSuccess();
          onClose();
        }, 800);
      ...
      const apiMsg = err.response?.data?.message || 'Failed to create user account.';
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="form-grid">
        ...
        {/* Employee Selection */}
        <FormField label="Select Employee" required fullWidth helperText="Choose an employee to grant ERP login access.">
          <Select
            name="employeeId"
          ...
          >
            ...
            {employees.map((emp) => {
              const hasAccount = existingUserEmployeeIds.has(emp._id);
              return (
                <option key={emp._id} value={emp._id} disabled={hasAccount}>
                  {emp.employeeCode} — {emp.fullName} ({emp.department?.departmentName || 'No Dept'}) {hasAccount ? ' [Account Exists]' : ''}
                </option>
              );
            })}
          </Select>
        ...
        {/* Username Input */}
        <FormField label="ERP Username" required fullWidth helperText="System login identifier (lowercase)">
          <Input
            name="username"
        ...
        <FormField label="Password" required fullWidth>
          ...
          <div style={{ border: '1px solid var(--neutral-300)', borderRadius: '6px', padding: '10px', backgroundColor: '#ffffff' }}>
            ...
            >
              {loadingData ? (
                ...
                <div style={{ padding: '12px', fontSize: '13px', color: 'var(--neutral-500)', textAlign: 'center' }}>
                  {roles.length === 0 ? 'No roles available.' : `No roles found matching "${roleSearchTerm}"`}
                </div>
              ) : (
                filteredRoles.map((r) => {
        ...
        <FormField label="Account Status" required fullWidth>
          ...
          >
            ...
            <option value="false">Inactive (Access Disabled)</option>
          </Select>
        </FormField>
      </form>
    </Modal>
