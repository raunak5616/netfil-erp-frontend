...
import { AlertCircle, CheckCircle2, Layers, Package, ShoppingCart } from 'lucide-react';

const OrderBOMFormModal = ({ isOpen, onClose, onSuccess }) => {
  // Option lists
  ...
  // 1. Initial load of eligible sales orders, stores, and plants
  useEffect(() => {
    if (!isOpen) return;
    setError('');
    setSelectedSOId('');
    ...
    Promise.all([
      ...
      .then(([soRes, storeRes, plantRes]) => {
        if (soRes.success && Array.isArray(soRes.salesOrders)) {
          // Filter confirmed or in_progress sales orders
      ...
      .catch((err) => {
        console.error('Failed to load eligible sales orders:', err);
        setError('Failed to load sales orders from server');
      })
      .finally(() => setLoadingOrders(false));
  }, [isOpen]);

  // 2. When Sales Order changes, fetch its line items
  useEffect(() => {
    if (!selectedSOId) {
      setSoItems([]);
      setSelectedSOItemId('');
  ...
  useEffect(() => {
    ...
    if (!parentItemId) {
      setError('Selected Sales Order Item has no associated parent Item');
      setReleasedBOMs([]);
      setSelectedMasterBOMId('');
      setPreviewBOM(null);
      return;
      ...
      .then((res) => {
        if (res.success && Array.isArray(res.boms)) {
          ...
          if (res.boms.length > 0) {
            ...
            setSelectedMasterBOMId('');
            setPreviewBOM(null);
            setError('No released Master BOM found for the parent item of this Sales Order Item');
          }
        }
      })
      .catch((err) => {
  ...
  // 4. When selected Master BOM changes, fetch its component items for preview
  useEffect(() => {
    ...
    >
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ fontSize: '12.5px', color: 'var(--neutral-600)', background: 'var(--neutral-50)', padding: '10px 12px', borderRadius: '4px', border: '1px solid var(--neutral-200)' }}>
          Order BOM generates an <strong>order-specific component snapshot</strong> derived from a released Master BOM for a confirmed Sales Order line item.
        </div>
        ...
        <div className="form-grid">
            ...
            >
              ...
              {salesOrders.map((so) => (
                <option key={so._id} value={so._id}>
                  {so.salesOrderNo} — {so.client?.companyName || 'Party'} ({so.status})
                </option>
              ))}
            </Select>
          </FormField>
          ...
          <FormField label="2. Select Order Item" required>
            <Select
        ...
        {/* Step 3: Selected Item Info & Master BOM selection */}
        {selectedSOItem && (
          <div style={{ background: '#fff', border: '1px solid var(--neutral-200)', borderRadius: '6px', padding: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', alignItems: 'center' }}>
              <div>
                <span style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--neutral-500)', fontWeight: 700 }}>
                  Selected Finished Parent Item
                </span>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--primary-700)' }}>
                  {orderQty} {selectedSOItem.uom?.uomCode || 'Units'}
                </div>
              </div>
            </div>

            <FormField label="3. Master BOM Reference" required>
              <Select
                ...
                {releasedBOMs.length === 0 ? (
                  ...
                  releasedBOMs.map((b) => (
                    <option key={b._id} value={b._id}>
                      {b.bomCode} (Version {b.version}) — Status: {b.status}
                    </option>
                  ))
                )}
              </Select>
        ...
        {previewBOM && previewBOM.items && previewBOM.items.length > 0 && (
          <div>
            <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--neutral-800)', marginBottom: '6px' }}>
              Component Snapshot Preview (Calculated for {orderQty} Units)
            </div>
...
export default OrderBOMFormModal;
